...
> **⚠️ 테스트 필수 규칙 (반드시 준수)**
> - API 연동 또는 비즈니스 로직이 포함된 **모든 Task**는 구현 완료 후 반드시 테스트를 수행해야 한다
> - 테스트 도구는 **Playwright MCP** (`mcp__playwright__*`)를 사용한다
> - 각 Task 내에 `### 🧪 테스트 체크리스트 (Playwright MCP)` 섹션을 **필수** 작성한다
> - 테스트 시나리오는 **정상 케이스 + 에러/엣지 케이스 + (해당 시) 보안 케이스**를 모두 포함해야 한다
> - 테스트 항목이 모두 통과해야만 해당 Task를 완료로 표시할 수 있다
> - 테스트 없이 Task를 완료로 표시하는 것은 **금지**된다
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - ### 🧪 테스트 체크리스트 (Playwright MCP)
    - [ ] **정상 케이스**: API 엔드포인트 정상 응답 확인 (200/201 상태 코드)
    - [ ] **에러 케이스**: 잘못된 입력 시 에러 응답 확인 (400/404/500)
    - [ ] **보안 케이스**: 인증되지 않은 요청 차단 확인 (401/403)
...
- **Task 006: 인증 및 권한 시스템 구현**
  ...
  - 보안 미들웨어 및 세션 관리
  - ### 🧪 테스트 체크리스트 (Playwright MCP)
    - [ ] **정상 케이스**: 올바른 인증 정보로 로그인 성공
    - [ ] **에러 케이스**: 잘못된 인증 정보로 로그인 실패
    - [ ] **보안 케이스**: 인증되지 않은 사용자가 보호된 페이지 접근 불가
    - [ ] **권한 케이스**: 낮은 권한 사용자가 관리자 기능 접근 불가
    - **수락 기준**: 위 테스트 항목 전체 통과 시 Task 완료
...
#### **🧪 테스트 검증 (필수 항목 — 미충족 시 ROADMAP 재작성)**

- [ ] API 연동 Task마다 `### 🧪 테스트 체크리스트 (Playwright MCP)` 섹션이 포함되었는가?
- [ ] 비즈니스 로직 Task마다 테스트 케이스가 명시되어 있는가?
- [ ] 각 테스트 섹션에 정상/에러/보안 케이스가 모두 포함되어 있는가?
- [ ] 각 Task의 수락 기준에 "테스트 전체 통과" 조건이 포함되어 있는가?
- [ ] 테스트 없이 완료로 표시된 Task가 없는가?
- [ ] 모든 사용자 플로우에 대한 E2E 테스트 시나리오가 정의되었는가?
- [ ] Phase 3에 통합 테스트 Task(Playwright MCP 활용)가 포함되었는가?
